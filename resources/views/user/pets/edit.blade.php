            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                <form action="{{ route('pets.update', $pet->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="type_id" class="form-select shadow appearance-none border rounded w-full py-2 px-3 text-gray-700
                            ...
                            @foreach($types as $type)
                                <option value="{{ $type->id }}" @if ($type->id == $pet->type_id) selected @endif>
                                    {{ $type->pet_type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" value="{{ $pet->pet_name }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700
